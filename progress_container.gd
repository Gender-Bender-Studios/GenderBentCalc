...
func _ready() -> void:
	...
	var Calc = $".."
	var totalPoints = Calc.TotalPoints
	...
	PointsLabel.size = Vector2(progressContainer.size.x/2,0)
	PointsLabel.position = Vector2(0,targetLabel.size.y) 
	PointsLabel.text = "Total Points: " + str(totalPoints)
